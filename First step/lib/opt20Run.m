...
if strcmpi(Solver,'LINPROG')
   ...
   if exist('linprog','file')
      ...
      Result.xState=(abs(x_k-Prob.x_L)<=xTol)+2*(abs(x_k-Prob.x_U)<=xTol);
      if ~isempty(Prob.A)
         bTol   = Prob.optParam.bTol;
         Ax     = Prob.A*Result.x_k(1:Prob.N);
         Result.bState = double((Ax<=Prob.b_L+bTol)+2*(Ax>=Prob.b_U-bTol));
      %else
      %   Result.bState = [];
      end
...
% 141219  hkh  Test equality using xTol when computing xState
% 141219  hkh  Add bState computation for linprog,quadprog,lsqlin,fmincon,lsqnonlin
