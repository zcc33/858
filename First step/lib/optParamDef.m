...
% Copyright (c) 1998-2012 by Tomlab Optimization Inc., $Release: 7.10.0$
% Written Sept 11, 1998.  Last modified Nov 24, 2012.
...
elseif strcmpi(Solver,'ego')
   ...
   EpsF       = 1E-2;     % Relative tolerance (f(x) - fOpt) <= EpsF * fOpt
   IterPrint  = 1;
elseif strcmpi(Solver,'rbfGlobal') | strcmpi(Solver,'multiCGO')
   SOLPar     = SOLGet('npsol',probType,nObj,nJac,m);
   SOLPar(27) = eps^(0.67);   % eps_Rank
   SOLPar(30) = 10000;    % Max iterations
   EpsF       = 1E-3;   % Relative tolerance (f(x) - fOpt) <= EpsF * fOpt
...
elseif strcmpi(Solver,'Tlsqr')
   ...
   SOLPar(30)=1000;
   SOLPar(1) = 0;
elseif strcmpi(Solver,'midaco')
   SOLPar = SOLGet('',probType,nObj,nJac,m);
   SOLPar(5) = 1000; % PriFreq
else
   % A SOL solver
...
% 121124  hkh  Add multiCGO and rbfGlobal
% 121202  hkh  Change findstr to strfind, findstr will be obsolete
