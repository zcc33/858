...
% Copyright (c) 1998-2013 by Tomlab Optimization Inc., $Release: 7.10.0$
% Written Oct 2, 1998.     Last modified Dec 24, 2013.
...
if abs(fx) >= 1E300
   ...
   if length(gTol) < n
      ...
      % gTol might have size > n if problems of different sizes are run
      h                   = gTol(1:n);
   end
   %xprint(x,'x:')
   x                    = x + 0.1*h.*ss;
...
% 121224  hkh  Safeguard size of gTol for case if abs(fx) >= 1E300
