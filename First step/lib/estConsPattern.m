...
if Prob.ConsDiff == 0 & ~isempty(Prob.FUNCS.dc)
   ...
   for i = 1:Trials
       x  = x_L+rand(N,1).*xD;
       if i==1 && isequal(Prob.FUNCS.c,'optim_cdc');
           % optim_cdc must be called once before optim_dc is used.
           optim_cdc(x, Prob);
       end
...
% 150504  rut  Utilize user-provided dc when called from fmincon.
