...
% Copyright (c) 1998-2011 by Tomlab Optimization Inc., $Release: 8.1.0$
% Written May 19, 1998.   Last modified Sept 16, 2014.
...
if PriLev > 0 & any(solvType == [ 9 10])
    ...
    if isfield(Result,'minPnts')
        if ~isempty(Result.minPnts)
            ...
            for i = 1:min(60,length(Result.minPnts))
                fprintf(' %d ',Result.minPnts(i));
                if mod(i,15) == 0, fprintf('\n'); end
            end
            if length(Result.minPnts) > 60
               fprintf(' and %d other points',length(Result.minPnts))
            end
            fprintf('\n');
        end
    end
...
elseif PriLev > 2 & ~isempty(x_0)
    % Print Level 3 gives printout of x_0 for global problems when running a local solver
    fprintf('Starting vector x');
    if size(x_0,1) > MAX_x
        fprintf(' (Length %d. Print first %d)',size(x_0,1), MAX_x);
    end
...
% 131227  hkh  Limit number of xPnt to 4 rows (60)
% 140916  hkh  Print level 3 gives x_0 for glb/glc problems, if defined
